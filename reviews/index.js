...
window.addEventListener("DOMContentLoaded",()=>{
    ...
    img.src = item.img;
    nameof.textContent = item.name;
    nameof.style.textTransform = "capitalize";
    about.textContent = item.job;
    about.style.textTransform = "uppercase";
    review.textContent = item.text;
});

let showReview = (currentItem)=>{
    let item = reviews[currentItem];
    img.src = item.img;
    nameof.textContent = item.name;
    nameof.style.textTransform = "capitalize";
...
next.onclick = ()=>{
    currentItem++;
    if(currentItem > reviews.length - 1 || currentItem < 0) {currentItem = 0};
    showReview(currentItem);
}
